...
public class DruidMycatRouteStrategy extends AbstractRouteStrategy {
	...
	public RouteResultset routeNormalSqlWithAST(SchemaConfig schema, String stmt, RouteResultset rrs, String charset,
		...
		if (schema.isNeedSupportMultiDBType()) {
			...
			if (stmt.indexOf("INFORMATION_SCHEMA.PROFILING WHERE QUERY_ID=") != -1) {
				System.out.println("错误语句2");
				stmt = "SELECT STATE AS `状态`, 1 AS `期间`, 2 AS `百分比` FROM INFORMATION_SCHEMA.PROFILING WHERE QUERY_ID=(select max(Query_ID) from INFORMATION_SCHEMA.PROFILING) GROUP BY STATE ORDER BY SEQ ";
			}
			System.out.println("sql语句=" + stmt);
		...
		// 分表
		/**
		 * subTables="t_order$1-2,t_order3" 目前分表 1.6 开始支持 幵丏 dataNode
		 * 在分表条件下只能配置一个，分表条件下不支持join。
		 */
		if (rrs.isDistTable()) {
	...
	private SQLExprTableSource getDisTable(SQLTableSource tableSource, RouteResultsetNode node)
	...
	// */
	// private static RouteResultset
	// findRouteWithcConditionsForOneTable(SchemaConfig schema, RouteResultset
	// rrs,
	// Map<String, Set<ColumnRoutePair>> conditions, String tableName, String
	// sql) throws SQLNonTransientException {
	...
	// LOGGER.debug("found partion nodes (using parent partion rule directly)
	// for child table to update "
	// + Arrays.toString(dataNodeSet.toArray()) + " sql :" + sql);
	// }
	// if (dataNodeSet.size() > 1) {
	// return RouterUtil.routeToMultiNode(rrs.isCacheAble(), rrs,
	// schema.getAllDataNodes(), sql);
	// } else {
	// rrs.setCacheAble(true);
	// return RouterUtil.routeToSingleNode(rrs, dataNodeSet.iterator().next(),
	// sql);
	// }
	...
	public RouteResultset routeSystemInfo(SchemaConfig schema, int sqlType, String stmt, RouteResultset rrs)
		...
		case ServerParse.SELECT:// if origSQL is like select @@
			...
			break;
		case ServerParse.DESCRIBE:// if origSQL is meta SQL, such as describe
									// table
	...
	private static RouteResultset analyseDescrSQL(SchemaConfig schema, RouteResultset rrs, String stmt, int ind) {
		...
		while (pos < stmt.length()) {
			...
			if (ch == '/' && pos + 4 < stmt.length() && stmt.charAt(pos + 1) == '*') {
				...
				// 匹配 [describe ]
				if (pos + MATCHED_FEATURE.length() < stmt.length()
						&& (stmt.substring(pos).toUpperCase().indexOf(MATCHED_FEATURE) != -1)) {
					pos = pos + MATCHED_FEATURE.length();
					break;
				} else if (pos + MATCHED2_FEATURE.length() < stmt.length()
						&& (stmt.substring(pos).toUpperCase().indexOf(MATCHED2_FEATURE) != -1)) {
